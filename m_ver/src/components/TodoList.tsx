...
export function TodoList({ todos, onToggle, onUpdate, onDelete, sortable = false, onReorder }: TodoListProps) {
  ...
  if (!sortable) {
    return (
      // 모바일 화면에서는 좌우 여백을 없애 목록이 화면 끝까지 꽉 차게 하고(-mx-4로 상위 컨테이너의
      // px-4를 상쇄), 넓은 화면(sm 이상)에서는 다시 카드 형태(둥근 모서리+테두리)로 보여준다.
      <ul className="-mx-4 divide-y divide-neutral-100 overflow-hidden border-y border-neutral-200 bg-white sm:mx-0 sm:rounded-xl sm:border-x">
        {items}
      </ul>
    )
  }
